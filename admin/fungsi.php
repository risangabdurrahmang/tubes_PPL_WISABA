<?php
    ...
    function editwst($data) {
        ...
        if(empty($_FILES['gambar_tour']['name']) && empty($_FILES['gambar_detail']['name']) && empty($_FILES['gambar_360']['name']) && empty($_FILES['qrcode']['name'])) {
            ...
            if($edit) {
                echo "<script>
                    alert('Berhasil');
                    document.location.href = 'wisata.php?id_wisata=".$id_wisata."';
                </script>";
            } else {
                echo "<script>
                    alert('Gagal');
    ...
    function editadm($data2) {
        ...
        $edit2 = $conn->query("UPDATE tabel_admin SET nama_admin = '$nama_admin', password = '$password' WHERE id_admin = '$id_admin'");
        if($edit2) {
            echo "<script>
                alert('Berhasil');
                document.location.href = 'admin.php?id_admin=".$id_admin."';
            </script>";
        } else {
            echo "<script>
                alert('Gagal');
